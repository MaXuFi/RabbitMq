...
@RefreshScope
@AllArgsConstructor
public class Fanout1Impl {

    @SneakyThrows
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue (value = RabbitConstant.FANOUT_QUEUE1, durable = "true"),
    ...
    public void helloWorldQueue(Message message, Channel channel){
        System.out.println("Fanout1Impl接收到消息：" + new String(message.getBody(), StandardCharsets.UTF_8));
